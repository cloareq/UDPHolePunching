...
int udpclient(int argc, char *argv[])
{
    ...
    tcp_serv = sock_create(lhost, lport, ipver, SOCK_TYPE_TCP, 1, 1, coming_port);
    ...
    while(running)
    {
        ...
        if(FD_ISSET(SOCK_FD(tcp_serv), &read_fds))
        {
            ...
            if(tcp_sock == NULL)
                continue;
            udp_sock = sock_create(phost, pport, ipver, SOCK_TYPE_UDP, 0, 1, coming_port);
            if(udp_sock == NULL)
            {
                sock_close(tcp_sock);
                sock_free(tcp_sock);
