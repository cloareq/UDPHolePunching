...
socket_t *sock_create(char *host, char *port, int ipver, int sock_type,
                      int is_serv, int conn, int coming_port)
{
    socket_t *sock = NULL;
...
int sock_connect(socket_t *sock, int is_serv)
{
    ...
    int reuse = 1;
    setsockopt(sock->fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(int));
    if(is_serv)
    {
    ...
    else
    {
      ...
      struct sockaddr_in sin = { 0 }; //init de la structure avec des 0
